...
<html lang="en">
    ...
    <body id="page-top">
        ...
        <div class="top">
            <div class="container">
                <a href="#" class="unpair" style="float: right;">UNPAIR</a>
                <div class="d-flex align-items-center justify-content-center top-main-wrapper">
                    <div class="top-data-wrapper" id="pending-pairing">
                        ...
                        <div class="row">
                            <div class="col text-center">
                                ...
                                <h2>CONNECTED</h2>
                                <br><br>
                                <h5>PIN required to configure the interfaces of this awetobot</h5>
                                <h5>Alternatively, you can also configure this awetobot on awetomate.io</h5>
                                <br><br>
                                <h4><strong>PIN can be found in <span style="color:red;">awetomate.io</span> > {{ $bot->site_name ?? '-' }} > awetobot</strong></h4>
                                <br><br>
        
                                <input type="text" name="authcode" id="authcode" placeholder="Input Auth Code">
                                <button id="btn-authcode"> submit </button>
                            </div>
                        </div>
                    </div>
        ...
        <script type="text/javascript">
            $(document).ready(function () {
                $(document).on('click','#btn-authcode', function(){
                    let authcode = $('#authcode').val();
                    let bot_id = "{{ $bot->id }}";
        
                    let data = 'authcode=' +authcode+ '&bot_id=' +bot_id;
        
                    $.post("{{ route('authentication') }}", data, function(result) {
                        if(result.status == 1){
                            location.reload();
                        }else{
                            alert('wrong code');
